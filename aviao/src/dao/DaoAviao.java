...
public class DaoAviao {
    public static boolean inserir(Aviao objeto) {
        String sql = "INSERT INTO aviao (modelo, capacidade, volume, data) VALUES (?, ?, ?, ?)";
        try {
    ...
    public static void main(String[] args) {
        ...
        objeto.setModelo("aviaoum");
        objeto.setCapacidade(123);
        objeto.setVolume(10.00);
        objeto.setData(LocalDate.parse("11/11/2001", DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    ...
    public static boolean alterar(Aviao objeto) {
        String sql = "UPDATE aviao SET modelo = ?, capacidade = ?, volume = ?, data = ? WHERE codigo=?";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
            ps.setString(1, objeto.getModelo());
            ps.setInt(2, objeto.getCapacidade());
    ...
    public static List<Aviao> consultar() {
        ...
        String sql = "SELECT codigo, modelo, capacidade, volume, data FROM aviao";
    ...
    public static Aviao consultar(int primaryKey) {
        //editar o SQL conforme a entidade
        String sql = "SELECT codigo, modelo, capacidade, volume, data FROM aviao WHERE codigo=?";
        PreparedStatement ps;
        try {
            ps = conexao.Conexao.getConexao().prepareStatement(sql);
